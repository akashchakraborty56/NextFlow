...
export const UploadImageNode = memo(function UploadImageNode({ id, data, selected }: NodeProps) {
  ...
  const handleUpload = useCallback(async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const { params, signature } = await api.getTransloaditParams('image');
      ...
      console.error('Transloadit upload failed:', err);
    } finally {
      setUploading(false);
    }
  ...
  return (
    <BaseNode id={id} label={d.label} icon={<ImagePlus size={14} />} color="blue"
      status={d.status} error={d.error} selected={selected ?? false}
      outputHandles={[{ id: 'image-out', label: 'Image' }]}
      outputs={d.outputs?.reduce((a, o) => ({ ...a, [o.key]: o.value }), {})}
    >
      {fileUrl ? (
        <div className="relative group">
          <img src={fileUrl} alt="Uploaded" className="w-full h-28 object-cover rounded-lg border border-zinc-700" />
        ...
        <label className="flex flex-col items-center justify-center h-24 border-2 border-dashed border-zinc-700 rounded-lg cursor-pointer hover:border-blue-500/50 transition-colors nodrag">
          <Upload size={18} className="text-zinc-500 mb-1" />
          <span className="text-[10px] text-zinc-500">{uploading ? 'Uploading...' : 'Click to upload image'}</span>
